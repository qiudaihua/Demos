...
public class DPL {
    ...
    private DPL(Activity a) {
        FrameLayout.LayoutParams lp = new FrameLayout.LayoutParams(
                LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
        FrameLayout layout = new FrameLayout(a);
        a.addContentView(layout, lp);
        mPL = new PL(a);
        layout.addView(mPL, LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
    }

    public static DPL instance(Activity a) {
        if (sDPL == null) {
            sDPL = new DPL(a);
        }
        return sDPL;
    ...
    public void markCoord(float x, float y) {
        if (mPL.getMode() != MODE_MARK) {
            mPL.setMode(MODE_MARK);
        }
        long now = SystemClock.uptimeMillis();
        MotionEvent event = MotionEvent.obtain(now, now,
                MotionEvent.ACTION_DOWN, x, y, 0);
        mPL.addPointerEvent(event);
        mPL.invalidate();
    }
    ...
    public void markView(View view) {
        ...
        markCoord(coord[0], coord[1]);
    }

    public class PL extends View implements InputDeviceListener {
        ...
        private final String ALT_STRATEGY_PROPERY_KEY = "debug.velocitytracker.alt";
        ...
        public PL(Context c, AttributeSet attrs, int defStyle) {
            this(c, attrs);
        }

        public PL(Context c, AttributeSet attrs) {
            this(c);
        }
        ...
        protected void addPointerEvent(MotionEvent event) {
            final int action = event.getAction();
        ...
        private boolean shouldLogKey(int keyCode) {
            switch (keyCode) {
        ...
        private final class FasterStringBuilder {
            private char[] mChars;
            private int mLength;
